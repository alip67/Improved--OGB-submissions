...
function main(PyA, k::Int64)
    ...
    # convert The csr_matrix python object to SparseMatrixCSC julia obeject
...
#      (Λ, V) = eigs(NRL, nev=k, tol=1e-6, ncv=2*k+1, which=:LM)
    ...
    invD = Diagonal(1.0 ./ (d .+ τ))
    Aop = LinearMap{Float64}(X -> A * X .+ (τ / N) * sum(X), N, N, isposdef=false, issymmetric=true)
    M = invD * Aop

    (Λ, V) = eigs(M, nev=k, tol=1e-6, ncv=2*k+1, which=:LM)

    Vinrm = Diagonal(1.0 ./ norm.(eachrow(V)))
    SCDM_V = Vinrm * V
